...
function loadSum() {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (partyAmountText !== "-") {
      const partyAmount =
        parseFloat(partyAmountText.replace(/[^0-9.-]+/g, "")) || 0;
    ...
    if (profitLossText !== "-") {
      const profitLoss =
        parseFloat(profitLossText.replace(/[^0-9.-]+/g, "")) || 0;
  ...
  try {
    ...
    data.forEach((e) => {
      ...
      row.innerHTML = `<tr>
                      ...
                      <td class="profit-loss" data-profit-loss="${profitLoss}">${
        profitLoss === "-"
          ? "-"
...
async function addBillPaymentAmount(billNum) {
  ...
  const response = await fetch(
    `/addBillPaymentAmount?billNum=${billNum}&billPaymentAmount=${billPaymentAmount}`
  );
...
function exportToPDF() {
  ...
  win.write("<hr>");
  win.write(
    tableContainer.innerHTML.replace(/<button[^>]*>.*?<\/button>/g, "")
  );
  win.write("<br>");
  win.write("</body></html>");
  win.close();
...
function exportToXLSX() {
  ...
  tableContainer.innerHTML = tableContainer.innerHTML.replace(
    /<button[^>]*>Notify<\/button>/g,
    ""
  );
  tableContainer.innerHTML = tableContainer.innerHTML.replace(
    /<button[^>]*>Add<\/button>/g,
