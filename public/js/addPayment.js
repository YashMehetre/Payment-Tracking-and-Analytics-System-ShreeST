...
const addPaymentData = async () => { 
    let data = {
        ...
        paymentMoreDetails : document.getElementById("paymentMoreDetails").value || ""
    }
    // console.log(data);
    if(data.vendorFirmName === "" || data.paymentDate === "" || data.paymentModeName === "" || data.paymentReceivedAmt === ""){
...
document.getElementById("addPaymentData").addEventListener("click",addPaymentData);
...
const showPaymentModesForSelect = async() =>{
 ...
 const showLastTenPaymentDetails = async () => {
    ...
    data.forEach((e) => {
        let amount = e.paymentReceivedAmt.toLocaleString('en-IN', {useGrouping:true})
