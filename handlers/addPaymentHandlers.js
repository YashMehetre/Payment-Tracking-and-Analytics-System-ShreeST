...
async function handleAddPaymentData(req, res) {
    const { vendorFirmName, paymentModeName, paymentDate, paymentReceivedAmt, paymentMoreDetails } = req.body;
    ...
    if (vendorId && paymentModeId) {
        ...
        try {
            const [result] = await pool.promise().execute(sql, [vendorId, paymentModeId, paymentDate, paymentReceivedAmt, paymentMoreDetails]);
...
module.exports = {handleDeletePaymentDetails,handleAddPaymentData,handleFetchPaymentModesForSelect,handleFetchPaymentDetails,handleFetchLastTenPaymentDetails,handleSearchPaymentData};